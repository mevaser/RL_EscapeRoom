...
class DQNRoom(GridWorldEnv):
    ...
    def step(self, action: int):
        ...
        # הוסף את מצב המכוניות הסופי למילון המידע
    ...
    def train_episode(self) -> float:
        ...
        while not done:
            ...
            # cars_state = [car["position"] for car in self.moving_cars] # <<< מחק את השורה הזו
            ...
            trajectory.append(
                {
                    "state": state,
                    "action": action,
                    "cars": info["cars_state"],  # <<< השתמש במצב המעודכן מ-info
                }
            )
        ...
        if (
            ...
            print(
                f"✅ Successful trajectory saved with reward: {total_reward} - ended at {tuple(state)}"
            )
    ...
    def plot_training_progress(self):
        ...
        plt.figure(figsize=(10, 5))
        eps = [
            max(self.epsilon_min, 1.0 * (self.epsilon_decay**i))
